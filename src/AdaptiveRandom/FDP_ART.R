...
# ArtPrioritizer.R                                                                                                             #
#                                                                                                                              #
# Implementation of the White Box Adaptive Random Testing (ART) prioritization algorithm.                                      #
#                                                                                                                              #
# Jonathan Miller Kauffman                                                                                                     #
#                                                                                                                              #
################################################################################################################################

# The "main" method used to call the prioritizer.  This function takes a
# coverage matrix, similarity metric, and furthest away metric as arguments.
# 
# The coverage matrix is a binary matrix.
# The columns of the matrix should correspond to test cases while the rows
# of the matrix should correspond to requirements.  A 1 in position i,j means
# that test case i covers requirement j, while a 0 means that test case i does
# not cover requirement j.
#
# The similarity metric is used to determine how close one test case is to
# another test case.  The available metrics are the Jaccard, Euclidean,
# Manhattan distances.
#
# The furthest away metric is used to determine which candidate test case
# is furthest away from all of the other test cases.  There are three
# possible values:
...
ART <- function(matrixName, similarityMetric, furthestAwayMetric, seed=100)
{
    set.seed(seed)
...
ART_real <- function(matrixName, similarityMetric, furthestAwayMetric)
{
	...
	while(!identical(untreatedOrdering, integer(0)))
	{
		...
		# Select a test case in the candidate set that is furthest away
		# from all of the test cases in the prioritized ordering using
		# the furthest away metric.  If this is the first iteration 
		# of this loop, there are no test cases in the prioritized
		# ordering, so randomly choose a test case from the candidate
		# set.
		if(currentTest == 1)
			testCase <- candidateSet[sample(1:length(candidateSet),1)]
		else
			testCase <- select(prioritizedOrdering, matrix, candidateSet, similarityMetric, furthestAwayMetric)

		# Remove the selected test case from the set of available test cases.
		untreatedOrdering <- untreatedOrdering[untreatedOrdering != testCase]
	...
	#write.table(prioritizedOrdering, file="prioritizedOrdering", row.names=FALSE, col.names=FALSE)
	return(list(Ord=prioritizedOrdering, Fit=APFD(prioritizedOrdering,makeLogFM(read.table(matrixName))), Pri="ART", SIM=similarityMetric, FAM=furthestAwayMetric))	
	#return(prioritizedOrdering)
}

# Modify ART slightly to make it a reducer.
ART_reduction <- function(matrixName, similarityMetric, furthestAwayMetric, Seed=100)
{
    set.seed(Seed)
    ART_reduction_real(matrixName, similarityMetric, furthestAwayMetric)
}
ART_reduction_real <- function(matrixName, similarityMetric, furthestAwayMetric, Seed)
{
	# Read in the coverage matrix.
	matrix <- read.table(matrixName)

    # Find the requirements covered by the entire test suite.
    testSuiteReqs <- as.logical(head(matrix, nrow(matrix) - 1)[[ncol(matrix)]])
	...
	# Keep looping until all of the test cases have been removed from
	# the original ordering.
	while((!identical(untreatedOrdering, integer(0))) && !identical(as.logical(reducedReqs), testSuiteReqs))
	{
		# A vector containing test cases that might be added to the
		# prioritized ordering.
		candidateSet <- generate(untreatedOrdering, matrix)
		...
		if(currentTest == 1)
			...
			testCase <- select(reducedTestSuite, matrix, candidateSet, similarityMetric, furthestAwayMetric)

        # Compute the requirements covered by the new reduced test suite.
	...
	# Write the prioritized ordering to file.
	#write.table(prioritizedOrdering, file="prioritizedOrdering", row.names=FALSE, col.names=FALSE)
	return(list(Ord=reducedTestSuite, Fit=APFD(reducedTestSuite,makeLogFM(read.table(matrixName))), Pri="ART_reduction", SIM=similarityMetric, FAM=furthestAwayMetric,Seed=Seed))	
	#return(prioritizedOrdering)
}

ART_Multi_Rep <- function(ART, similarityMetric, furthestAwayMetric, fFM, Trials=10, Par=FALSE, RandSeed=100)
{
    set.seed(RandSeed)
    ART_Multi_Rep_real(ART, similarityMetric, furthestAwayMetric, fFM, Trials, Par)
}
ART_Multi_Rep_real <- function(ART, similarityMetric, furthestAwayMetric, fFM, Trials=10, Par=FALSE)
{
  #set.seed(RandSeed)
  ParamMatrix <- expand.grid(fFM=fFM, ART=ART, similarityMetric=similarityMetric, furthestAwayMetric=furthestAwayMetric, Trials=Trials, 
	...
	resDF <- adply(ParamMatrix, 1, function(pDF) 
  {
    ...
    pList <- list(matrixName=pDF$fFM, similarityMetric=pDF$similarityMetric, furthestAwayMetric=pDF$furthestAwayMetric, ART=pDF$ART)
...
# This method takes in the available test cases and the coverage matrix and
# returns a candidate set of vectors that can be added to the prioritized
# ordering.
generate <- function(untreatedOrdering, matrix)
{
	...
	while(TRUE)
	{
		...
		# If this test case does not cover any new requirements, return
		# the candidate set.
		if(identical(totalRequirements | newRequirements, totalRequirements))
			return(candidateSet)
		...
		candidateSet <- union(candidateSet, untreatedOrdering[testCase])
	}
}

# This method chooses a test case from the candidate set that is furthest
# away from all of the test cases in the prioritized ordering.
select <- function(prioritizedOrdering, matrix, candidateSet, similarityMetric, furthestAwayMetric)
{
	# A matrix with one row for each test case in the prioritized ordering
	# and one column for each test case in the original ordering.  A position
	# i,j in the matrix contains the distance from test case i to test case
	# j.  As of right now, the distance between two test cases is defined
	# using the Jaccard distance.
	distanceMatrix <- as.data.frame(mat.or.vec(length(prioritizedOrdering), length(candidateSet)))
	...
	for(i in 1:length(prioritizedOrdering))
		for(j in 1:length(candidateSet))
		{
			if(similarityMetric == "JaccardDistance")
				distanceMatrix[[j]][i] <- JaccardDistance(prioritizedOrdering[i], candidateSet[j], matrix)
			else if(similarityMetric == "EuclideanDistance")
				distanceMatrix[[j]][i] <- EuclideanDistance(prioritizedOrdering[i], candidateSet[j], matrix)
			else if(similarityMetric == "ManhattanDistance")
				distanceMatrix[[j]][i] <- ManhattanDistance(prioritizedOrdering[i], candidateSet[j], matrix)
		}
	
	# Choose the test case in the candidate set that is furthest away from
	# the test cases in the prioritized ordering.
	testCase <- FurthestTestCase(distanceMatrix, furthestAwayMetric)
	return(candidateSet[testCase])
}

# Returns the Jaccard distance between two test cases.  The Jaccard distance
# is defined by 1 - |A n B| / |A U B|.
JaccardDistance <- function(testCase1, testCase2, matrix)
{
	...
	# Compute the intersection of the requirements covered by each test case.
...
# Returns the Euclidean distance between two test cases.  The Euclidean distance
# is defined as sqrt[(q1-p1)^2 + (q2-p2)^2 + ... + (qn-pn)^2].
...
# Returns the Manhattan distance between two test cases.  The Manhattan distance
# is like the Euclidean distance, except that a grid-like path must be followed.
# It is the the Euclidean distance squared in this context, and it will
# always be a nonnegative integer.
...
# Determines which test case in the candidate set is furthest away from
# the test cases in the prioritized ordering.
FurthestTestCase <- function(distanceMatrix, furthestAwayMetric)
{
	...
	if(furthestAwayMetric == "min")
	{
		...
		for(i in 1:ncol(distanceMatrix))
		{
			...
			# in the matrix.
			if(distanceMatrix[[i]][min] > distanceMatrix[[max]][maxMin])
			{
				max <- i
				maxMin <- min
			}
	...
	else if(furthestAwayMetric == "max")
	{
		...
		for(i in 1:ncol(distanceMatrix))
		{
			...
			# in the matrix.
			if(distanceMatrix[[i]][max2] > distanceMatrix[[max1]][maxMax])
			{
				max1 <- i
				maxMax <- max2
			}
