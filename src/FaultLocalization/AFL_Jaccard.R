...
runJaccard <- function(lFM, failingTests, liveTests)
{
    ...
    suspiciousnessConfidence <- calculateSuspiciousnessJaccard(passRatio=passFailRatio$PassRatio,failRatio=passFailRatio$FailRatio,totalLivePass=passFail$TotalLivePass,totalLiveFail=passFail$TotalLiveFail)
...
# test cases.
calculateSuspiciousnessJaccard <- function(passRatio, failRatio, totalLivePass, totalLiveFail)
{
    ...
    for(i in 1:numStmts)
    {
        ...
        else if((failRatio[i]==1 || failRatio[i]==0) && passRatio[i]==0)
        {
            suspiciousness[i] <- -1
        }
        ...
        {
            suspiciousness[i] <- failRatio[i] / ((1 - failRatio[i]) + passRatio[i])
        }
    }
