...
Rand_Multi_Rep <- function(Rand, fFM, cPopSize, Trials=10, RandSeed=100, Par=FALSE)
{
    set.seed(RandSeed)
    Rand_Multi_Rep_real(Rand, fFM, cPopSize, Trials, RandSeed, Par)
}
Rand_Multi_Rep_real <- function(Rand, fFM, cPopSize, Trials=10, RandSeed=100, Par=FALSE)
{
  #set.seed(RandSeed)
  
  ParamMatrix <- expand.grid(Rand=Rand, fFM=fFM, cPopSize=cPopSize, Trials=Trials,
  ...
  print(c("Configurarions to be executed: ", ParamMatrix))
    ...
    pList <- list(Rand=pDF$Rand, lFM=makeLogFM(read.table(pDF$fFM)), cPopSize=pDF$cPopSize)
    ...
    batchDF <- ldply(runif(pDF$Trials, 1, 1000), function(tSeed, argList)
    {
      ...
      rTime <- system.time(expOut <- do.call(argList$Rand, args=argList[-1]))[[3]]
...
Rand_POP_real <- function(lFM, cPopSize, Seed=100)
{
  #print(RandomSeed)
	...
	# Create a list of the fittness scores, corresponding to the orderings
	#  This works nicely since the orderings are stored by column 
	...
	return(list(Ord=Best, Fit=BestFit, Pri="Rand_POP", Conf=cPopSize, Seed=Seed))	
}
...
Rand_POP_reduction_real <- function(lFM, cPopSize, Seed=100)
{
  #print(RandomSeed)
	...
	# Return the best individual and it's Fit score
	return(list(Ord=Best, Fit=BestFit, Pri="Rand_POP_reduction", Conf=cPopSize, Seed=Seed))	
}
...
Rand_LIN_real <- function(lFM, cPopSize=NA, Seed=100)
{
	...
	# If availableTestCases[i] = 1, test case i has not yet been added to the prioritized ordering.
	...
	return(list(Ord=prioritizedOrdering, Fit=APFD(prioritizedOrdering,lFM), Pri="Rand_LIN", Conf=cPopSize, Seed=Seed))
}
...
Rand_LIN_reduction_real <- function(lFM, cPopSize=NA, Seed=100)
{
	...
	while(!identical(reducedReqs, testSuiteReqs))
	{
		...
		if(availableTestCases[candidateTestCase])
		{
            ...
            # Re-calculate the requirements covered by the reduced test suite.
	...
	#write.table(prioritizedOrdering, file="prioritizedOrdering", row.names=FALSE, col.names=FALSE)
	#return(prioritizedOrdering)
	return(list(Ord=reducedSuite, Fit=APFD(reducedSuite,lFM), Pri="Rand_LIN_reduction", Conf=cPopSize, Seed=Seed))
}
