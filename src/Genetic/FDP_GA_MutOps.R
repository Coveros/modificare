...
MO_DM <- function(Ordering)
{
	...
	insertPosition <- as.integer(runif(1, 0, length(Ordering[-displace])))
	# now the actual work	
	t1 <- append(Ordering[-displace], Ordering[displace], insertPosition)
...
MO_ISM <- function(Ordering)
{
	...
	t1 <- append(Ordering[-targetLoc], Ordering[targetLoc], insertPoint)
...
MO_IVM <- function(Ordering)
{
	...
	insertPosition <- as.integer(runif(1, 0, length(Ordering[-displace])))
	# now the actual work	
	t1 <- append(Ordering[-displace], Ordering[rev(displace)], insertPosition)
...
MO_SIM <- function(Ordering)
{
	...
	subStringLength <- sample(c((as.integer(numberOfTests/4)):(as.integer(numberOfTests/2))), 1)

	# Random location to begin the displacement substring
	subStringLocation <- sample(c(1:(numberOfTests-subStringLength)), 1)

	# Initialize the new ordering 
	newOrder <- Ordering
	
	# now invert the substring
	newOrder[subStringLocation:(subStringLocation+subStringLength)] <- newOrder[rev(subStringLocation:(subStringLocation+subStringLength))]
...
MO_SM <- function(Ordering)
{
	...
	newOrder[subStringLocation:(subStringLocation+subStringLength)] <- 
		newOrder[sample(subStringLocation:(subStringLocation+subStringLength))]
