...
GA_Multi_Rep <- function(fFM, cPopSize=100, cChildDens=0.2, cMutRate=0.2,
                    CrossOp="CO_OX1", MutOp="MO_SIM", SelOp="SO_TRU",
                    TransOp="TO_LIN", TermCond="TermStag", EndCond=25,
                    Trials=10, RandSeed=100, Par=FALSE)
{
    set.seed(RandSeed)
    GA_Multi_Rep_real(fFM,cPopSize,cChildDens,cMutRate,CrossOp,MutOp,
                      SelOp,TransOp,TermCond,EndCond,Trials,Par)
}
GA_Multi_Rep_real <- function(fFM, cPopSize=100, cChildDens=0.2, cMutRate=0.2, 
                         CrossOp="CO_OX1", MutOp="MO_SIM", SelOp="SO_TRU", 
                         TransOp="TO_LIN", TermCond="TermStag", EndCond=25, 
                         Trials=10, Par=FALSE) 
{
  ...
  ParamMatrix <- expand.grid(fFM=fFM, cPopSize=cPopSize, cMutRate=cMutRate, 
                             cChildDens=cChildDens, CrossOp=CrossOp, MutOp=MutOp, 
                             SelOp=SelOp, TransOp=TransOp, Trials=Trials, 
	...
	resDF <- adply(ParamMatrix, 1, function(pDF)
  {
    ...
    batchDF <- ldply(runif(pDF$Trials, 1, 1000), function(tSeed, argList)
    {
      ...
      rTime <- system.time(expOut <- do.call("GA_real", args=argList))[[3]]
...
# Seed
GA <- function(lFM, cPopSize=100, cChildDens=0.2, cMutRate=0.2, CrossOp="CO_OX1",
                MutOp="MO_SIM", SelOp="SO_TRU", TransOp="TO_LIN",
                TermCond="TermStag", EndCond=25, Seed=100)
{
    set.seed(Seed)
    GA_real(lFM,cPopSize,cChildDens,cMutRate,CrossOp,MutOp,SelOp,TransOp,
            TermCond,EndCond,Seed)
}
GA_real <-function(lFM, cPopSize=100, cChildDens=0.2, cMutRate=0.2, CrossOp="CO_OX1",
              MutOp="MO_SIM", SelOp="SO_TRU", TransOp="TO_LIN", 
              TermCond="TermStag", EndCond=25, Seed)
{
  #set.seed(Seed)
	nT <- ncol(lFM)
	...
	while(do.call(TermCond, args=TermList))
  {
    ...
    Children <- GetChildren(Pop, iParents, CrossOp=CrossOp)
		...
		Children[,iTargetMuts] <- apply(Children[,iTargetMuts], 2, function(x) 
      { 
        do.call(MutOp, args=list(x))
      })
		dimnames(Children) <- NULL
    
		# Get Child fit, select survivors, construct population, get fitness
	...
	return(list(Ord=Best, Fit=BestFit, cPopSize=cPopSize, cChildDens=cChildDens, 
             cMutRate=cMutRate, CrossOp=CrossOp, MutOp=MutOp, SelOp=SelOp, 
             TransOp=TransOp, TermCond=TermCond, EndCond=EndCond, 
...
{ 
  ...
  childPop <- matrix(unlist(childPop), nrow=nrow(Pop), ncol=(length(cParents)))
