...
GA_Multi_Rep <- function(fFM, cPopSize=100, cChildDens=0.2,
...
GA_Multi_Rep_real <- function(fFM, cPopSize=100, cChildDens=0.2,
                     cMutRate=0.2, CrossOp="CO_OX1", MutOp="MO_SIM",
                     ...
                     Par=FALSE) 
{
  ...
  ParamMatrix <- expand.grid(fFM=fFM, cPopSize=cPopSize,
    ...
    resDF <- adply(ParamMatrix, 1, function(pDF)
  {
               ...
               function(tSeed, argList)
    {
      ...
      rTime <- system.time(expOut <- do.call("GA_real",
               args=argList))[[3]]
