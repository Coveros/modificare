...
#                                                                                                                              #
# All of this code was either written by Zachary Douglas Williams or some other                                                #
# author and is taken from the kanonizo project.                                                                               #
#                                                                                                                              #
...
GRD_real <- function(coverageMatrixName, timingsFileName)
{
	...
	tempCoverageReport <- try(
	...
	#if(tempCoverageReport[1] != "Error in file(file, \"rt\") : cannot open the connection\n")	
        #{
	  ...
	  # rename all the tests, matching to numbers so the first test is 1, then 2, etc.
	  NameTTD <- c(1:(length(tempTimingInfo$Time)))

	  tempTimingReport <- cbind(NameTTD, subset(tempTimingInfo, select="Time"))

      # run prioritization (results contain new ce value and prioritized test suite 
	  tempResultsGreedy <- RunGreedyPrioritize(tempCoverageReport, tempTimingReport)
	...
	#	quote=FALSE)
	#}
	return(list(Ord=tempResultsGreedy, Fit=APFD(tempResultsGreedy,makeLogFM(read.table(coverageMatrixName))), Pri="GRD"))	
...
#
############################################
GRD_reduction <- function(coverageMatrixName, timingsFileName, seed=100)
{
    set.seed(seed)
...
GRD_reduction_real = function(coverageMatrixName, timingsFileName)
{
	...
	tempCoverageReport <- try(
      ...
      # run prioritization (results contain new ce value and prioritized test suite 
	  tempResultsGreedy <- RunGreedyReduce(tempCoverageReport, tempTimingReport)
	...
	#	quote=FALSE)
	#}
	return(list(Ord=tempResultsGreedy, Fit=APFD(tempResultsGreedy,makeLogFM(read.table(coverageMatrixName))), Pri="GRD_reduction"))	
    #return(tempResultsGreedy)
}


GRD_Multi_Rep <- function(GRD, fFM, fTF, Trials=10, Par=FALSE, RandSeed=100)
{
    set.seed(RandSeed)
...
GRD_Multi_Rep_real <- function(GRD, fFM, fTF, Trials=10, Par=FALSE)
{
	...
	resDF <- adply(ParamMatrix, 1, function(pDF) 
  {
    ...
    pList <- list(coverageMatrixName=pDF$fFM, timingsFileName=pDF$fTF, GRD=pDF$GRD)
...
findNewReq = function(reqs, tempTests)
{
   ...
   for(tj in tempTests)
   {
      tjReq <- unlist(subset(reqs, reqs$NameCD==tj, select=Requirements))
	...
	(length(ReqsCovered) != 0))
      {
         ...
         # Jonathan Miller Kauffman - Made infinity because with nanosecond
         # timings, the timings were larger than the "really big number".
         EfficiencyRatio <- Inf
	    ...
	    tiReq <-subset(Reqs, NameCD==ti, select=c(Requirements))$Requirements
	    ...
	    numberNew <- numberNotShared(ReducedReqsCovered, tiReq)
	       ...
	       #arithmetic won't work so set ratio to inital ratio to skip test
...
ConvertCoverageSummaryRandomMatrix = function(CoverageMatrix)
{
  ...
  #
  # Jonathan Miller Kauffman - Although the comment said to account for
  # the summary row and column, the code did not.  Subtracted one from
  # the number of tests and requirements because of the summary row
  # and column.
