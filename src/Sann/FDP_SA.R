...
SANN_Multi_Rep <- function(fFM, NF="NF_FS", AF="AF_kirklin", CF="CF_kirklin", Trials=10,
                        RandSeed=100, Par=FALSE)
{
    set.seed(RandSeed)
    SANN_Multi_Rep_real(fFM, NF, AF, CF, Trials, RandSeed, Par)
}
SANN_Multi_Rep_real <- function(fFM, NF="NF_FS", AF="AF_kirklin", CF="CF_kirklin", Trials=10,
                           RandSeed=100, Par=FALSE)
{
  #set.seed(RandSeed)
  
	ParamMatrix <- expand.grid(fFM=fFM, NF=NF, AF=AF, CF=CF, Trials=Trials, 
  ...
  print(c("Configurations to be executed: ", ParamMatrix))
	...
	{
    pList <- list(lFM=makeLogFM(read.table(pDF$fFM)), NF=pDF$NF, AF=pDF$AF, 
                  CF=pDF$CF)
    ...
    batchDF <- ldply(runif(pDF$Trials, 1, 1000), function(tSeed, argList)
    {
      ...
      rTime <- system.time(expOut <- do.call("SANN_real", args=argList))[[3]]
...
## Bare-Bones simulated annealing algorithm as described in the Kirklin thesis
SANN <- function(lFM, NF="NF_FS", AF="AF_kirklin", CF="CF_kirklin", itLimit=25,
                Seed=100)
{
    set.seed(Seed)
    SANN_real(lFM, NF, AF, CF, itLimit, Seed)
}
SANN_real <- function(lFM, NF="NF_FS", AF="AF_kirklin", CF="CF_kirklin", itLimit=25,
                 Seed=100)
{	
	#set.seed(Seed)
	...
	# set the current neighbor to the initial sequence, and calc fitness
	...
	while(itSinceImprovement < itLimit) 
	{
		...
		} else if(AcceptChance < do.call(AF, args=list(DeltaFitness=abs(NearOrdFit - 
	...
	return(list(Ord=CurrOrd, Fit=CurrOrdFit, Pri="SANN", Conf=NF, 
             TotalIt=totalIterations,Seed=Seed, Temp=currentTemperature))
}
